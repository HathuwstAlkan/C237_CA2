...
const checkAuthenticated = (req, res, next) => {
    ...
    return res.redirect('/'); // Redirect to the new landing page (registration.ejs)
};
...
/* ==============================
    Routes
    ============================== */
// Home (landing with dynamic partial loader) - NOW RENDERS REGISTRATION PAGE
app.get('/', (req, res) => {
    res.render('registration'); // Changed from 'index' to 'registration'
});
...
app.post('/register', async (req, res) => {
    try {
        const {
            ...
            address,
            contact,
            phone_number // This might be from the form directly as 'contact' or 'phone_number'
        } = req.body;

        // Keep required fields minimal; password only needs to be present
        const phone = phone_number || contact || null; // Use phone_number if present, else contact
        const errors = [];
        ...
        await db.query(
            `INSERT INTO customers
             ...
             VALUES (?, ?, 'regular', ?, ?, ?, ?, ?)`,
            [username, password_hash, first_name, last_name, email, phone, address || null]
        );

        return res.json({
            success: true,
            message: 'Registration successful! Please log in.',
            redirectUrl: '/' // your index.ejs (now registration.ejs) will handle loading the login partial
        });
    } catch (err) {
